---
title: 前端程序员常用工具
date: 2021-08-23 00:00:00
permalink: /pages/2b5ed1/
categories: 
  - other
tags: 
  - 
---

## 代码编辑器

- [Visual Studio Code](https://code.visualstudio.com/)
- [Sublime Text](https://www.sublimetext.com/)
- [WebStorm](https://www.jetbrains.com/webstorm/)

## 浏览器

- [Microsoft Edge](https://www.microsoft.com/zh-cn/edge/download?form=MA13FJ)
- [Google Chrome](https://www.google.com/intl/zh-CN/chrome/)

## Node 相关工具

- [NPM (Node Package Manager)](https://www.npmjs.com/)
- [Yarn](https://yarnpkg.com/)
- [Pnpm](https://pnpm.io/)
- NVM，安装步骤参考这篇 [NVM 的安装与使用](/article/other/nvm.html)
- NRM，安装步骤参考这篇 [NRM 的安装与使用](/article/other/nrm.html)

## 版本控制工具

- [Git](https://git-scm.com/)

## 管理/切换 Host

- [SwitchHost](https://switchhosts.vercel.app/zh)

## 邮箱

- [Outlook](https://outlook.live.com/)
- [Foxmail](https://www.foxmail.com/)

## 待办工具

- [Microsoft To Do](https://to-do.office.com/tasks/)
- [Todoist](https://todoist.com/zh-CN)

## Markdown 编辑器

- [Typora](https://typora.io/)
- [Obsidian](https://obsidian.md/)

## 翻墙工具、机场

- [Clash 下载](https://github.com/Fndroid/clash_for_windows_pkg/releases)
- [Clash 文档](https://docs.cfw.lbyczf.com/)
- [奈云](https://www.v2ny.com/)
- [一元机场](https://xn--4gq62f52gdss.com)
